...
import {
 StyledColors, StyledColor, StyledColorContainer, StyledRow, StyledRows,
} from './StyledColors';
...
const buildState = (props: IColorsProps): IColorsState => {
  ...
  const colorsPerRow = columns
    || (colorRows.length === 1 ? Math.sqrt(colorRows[0].colors.length)
      : colorRows.reduce((res, row) => (
          Math.min(res, row.colors.length)
...
class Colors extends Component<IColorsProps, IColorsState> {
  ...
  render() {
    const {
      ...
      wrap, onSelect, columns, size, ...rest
    } = this.props;
